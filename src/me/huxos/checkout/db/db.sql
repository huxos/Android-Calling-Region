...
/*短信关键字白名单*/
create table  if not exists sms_keyword_whitelist(no integer primary key, keyword text, phone_number text, enable integer);
/*短信关键字黑名单*/
create table  if not exists sms_keyword_blacklist(no integer primary key, keyword text, phone_number text, enable integer);
/*拦截电话日志*/
